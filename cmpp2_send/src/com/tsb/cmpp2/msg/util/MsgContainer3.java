...
import com.tsb.cmpp2.msg.domain.*;
 ...
 */
public class MsgContainer3 {
	private static Log log = LogFactory.getLog(MsgContainer3.class);
	...
	public static DataInputStream getSocketDIS() {
		 try {
			 in = new DataInputStream(MsgContainer3.getSocketInstance()
			 .getInputStream());
			 } catch (IOException e) {
			 in = null;
		 }
	...
	public static DataOutputStream getSocketDOS() {
		 try {
			 out = new DataOutputStream(MsgContainer3.getSocketInstance()
			 .getOutputStream());
			 } catch (IOException e) {
			 out = null;
		 }
...
//				}
	...
	public static  boolean sendMsg(String msg,String cusMsisdn){
		try{
			...
			log.info("1  消息内容转bytes："+bytes.toString());
			log.info("2  消息内容转bytes长度："+bytes.length+"");
			//短短信
...
*/
	...
	private static boolean sendShortMsg(String msg, String cusMsisdn) {
		try {
			...
			msgSocket = MsgContainer3.getSocketInstance();
			...
			while (!result) {
				...
				if (count >= (MsgConfig.getConnectCount() - 1)) {
					MsgContainer3.setFlag(false);
					MsgContainer3.cancelISMG();
					MsgContainer3.getSocketInstance();
				}
			}
			boolean success = sender.start();
			if (success) {
