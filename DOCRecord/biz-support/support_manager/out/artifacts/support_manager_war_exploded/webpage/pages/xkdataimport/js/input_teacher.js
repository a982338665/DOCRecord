...
$(function(){
    ...
    $("#sub").click(function(){
        ...
        var phone=$("#phone").val();
        if(schoolId && schoolId!=''&&teacherName&&phone&&teacherName!=''
            &&phone!=''&&phone.length==11&&phone){
            var s=$("#fun").searchFormatSeentao();
            var sj = $.jsonToString(s);

            $.ajaxSeentao($.getRootPath()+"/xkUser/inputTeacher.rest",sj,function(json){

                if(json.code===1){

                    ischool.layout.success(json.data,$.getRootPath()+'/webpage/pages/xkdataimport/input_teacher.html');
