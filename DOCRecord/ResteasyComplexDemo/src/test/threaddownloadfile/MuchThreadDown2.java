 ...
 *
 */
public class MuchThreadDown2 {

//  private String path = "http://mpge.5nd.com/2016/2016-11-15/74847/1.mp3";    //下载路径
    private static String path = "http://www.manyjar.com/download?storeName=m/mahout-examples/mahout-examples-0.8-job.jar";
    private static String targetFilePath="C:\\Users\\cjh\\Desktop";  //下载文件存放目录
     ...
     */
    public MuchThreadDown2(String path, String targetFilePath, int threadCount) {
    	if(path!=null){
    		 this.path = path;
    	}
    ...
    public void download() throws Exception{
        ...
        if(code == 200){
            ...
            for(int threadId = 0; threadId < threadCount; threadId++){//为每个线程分配任务
                ...
                DownloadThread.getInstance().setThreadId(threadId);
                DownloadThread.getInstance().setStartIndex(startIndex);
                DownloadThread.getInstance().setEndIndex(endIndex);
                DownloadThread.getInstance().start();//开启线程下载
...
//          randomAccessFile.close();
    ...
    //删除线程产生的临时文件
    private synchronized static void cleanTemp(File file){
        file.delete();
    }
    ...
    public static void main(String[] args) {
        try {
            new MuchThreadDown2(null, null, 3).download();
  ...
  //下载的线程
    static class DownloadThread extends Thread{
        ...
        private static  ThreadLocal<DownloadThread> dow=new ThreadLocal<MuchThreadDown2.DownloadThread>();

        public static  DownloadThread getInstance(){
        	DownloadThread dowT=dow.get();
        	if(dowT==null){
        		dowT=new DownloadThread();
        		dow.set(dowT);
        	}
        	return dowT;
        }
