...
/**
 * 切面MethodCacheInterceptor，这是用来给不同的方法来加入判断如果缓存存在数据，
 * 从缓存取数据。否则第一次从数据库取，并将结果保存到缓存中去。
 * @author lishengbo
 * @Time 2017年12月4日下午2:03:52
 */
...
//               "D:\\lunaJee-workspace\\msm\\msm_core\\src\\main\\java\\com\\mucfc\\msm\\common\\cacheConf.properties");  
         ...
         xxxRecordManagerTime = Long.valueOf(p.getProperty("pers.lishbo.aservice.commonservice.impl.CutDataSourceServiceImpl"));  
//         xxxSetRecordManagerTime = Long.valueOf(p.getProperty("pers.lishbo.aservice.commonservice.impl.UserServiceImpl"));  
         xxxSetRecordManagerTime = Long.valueOf(p.getProperty("pers.lishbo.aservice.abstractservice.impl.GanSuServiceImpl"));  
         // 创建list  
     ...
     try {  
         ...
         if (value != null) {  
             ...
             new Thread(new Runnable() {  
                 @Override  
                 public void run() {  
                     if (tkey.startsWith("pers.lishbo.aservice.commonservice.impl.CutDataSourceServiceImpl")) {  
                         redisUtil.set(tkey, tvalue, xxxRecordManagerTime);  
                     } else if (tkey.startsWith("pers.lishbo.aservice.abstractservice.impl.GanSuServiceImpl")) {  
