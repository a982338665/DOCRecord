...
public class JXLExcelUtil {
	...
	public static WritableWorkbook createFile(String savePath,String sheetName,Integer sheetNum) {
		...
		try {
			...
			sheet = wwb.createSheet(sheetName,sheetNum);
			System.out.println(savePath+":"+"第"+sheetNum+"个工作表("+sheetName+")");
		} catch (Exception e) {
			System.out.println("创建Excel文件或设置sheet名称异常：" + e.getMessage());
		}
		return null;
	...
	public static void main(String[] args) {
		...
		// 输出的excel的路径
		String filePath = "C:\\Users\\cjh\\Desktop\\test1.xls";
		try {
			// 获得开始时间
			...
			for (int i = 0; i < title.length; i++) {
				//i代表列(0代表第一列)，0代表第一行，以上行列确定后所填写的内容
			...
			jxl.write.WritableCellFormat wcf = new jxl.write.WritableCellFormat(
					nf);
			// 填充产品价格
			jxl.write.Number nb = new jxl.write.Number(2, 1, 2.45678, wcf);
			//-----------------------------------------------------------
